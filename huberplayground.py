...
class QuantileHuberPlayground(BaseDatafit):
...
class SmoothQuantileRegressorPlayground(BaseEstimator, RegressorMixin):
    ...
    def fit(self, X, y):
        """Fit using adaptive smoothing: delta is reduced when converged."""
        w = np.zeros(X.shape[1])
        delta = self.delta_init

        if self.verbose:
            print(
                f"Adaptive smoothing: delta {self.delta_init:.2e} --> "
                f"{self.delta_final:.2e} (adaptive steps)")

        datafit = QuantileHuberPlayground(
            quantile=self.quantile, delta=delta)
        ...
        while delta > self.delta_final:
            ...
            # Check convergence (relative change in coefficients)
            rel_change = np.linalg.norm(w - prev_coef) / \
                (np.linalg.norm(prev_coef) + 1e-12)
            if self.verbose:
                residuals = y - X @ w
